...
splitTracks <- function(tracks, splitlength) {
  i <- 0
  points <- NULL
  lineslist <- NULL
  for(track in tracks@lines) {
    for(linesegs in track@Lines) {
      str(linesegs)
      cds <- coordinates(linesegs)
      d <- diag(spDists(cds)[-1,-nrow(cds)])
      splits <- which(d > splitlength)
      start <- 1
      linelist <- NULL
      for(seg in splits) {
        segment <- cds[start:seg,,drop=FALSE]
        str(segment)
        if(nrow(segment) > 1) {
          newseg <- Line(segment)
          linelist <- c(linelist, newseg)
        } else {
          points <- rbind(points, segment)
        }
        start <- seg+1
      }
      segment <- cds[start:nrow(cds),,drop=FALSE]
      str(segment)
      if(nrow(segment) > 1) {
        newseg <- Line(segment)
        linelist <- c(linelist, newseg)
  ...
  if(!is.null(points)) {
    spoints <- SpatialPoints(points, proj4string=tracks@proj4string)
  } else {
    spoints <- NULL
  }

  list(tracks=SpatialLines(lineslist, proj4string=tracks@proj4string),
       points=spoints)
}


getTrackLength <- function(track) {
  len <- 0
  if(nrow(track) >= 2) {
...
insertTracks <- function(sortedTracks, newTracks) {
  sortTracks(c(newTracks, sortedTracks))
}

SIMPLIFY <- FALSE
simplifyTrack <- function(track) {
  len <- getTrackLength(track)
  if(len < mintracklen) { ## Convert short tracks to points
    point <- data.frame(lon=mean(track$lon), lat=mean(track$lat))
    list(point)
  } else if(SIMPLIFY && nrow(track) >= 3) {
...
simplifyTracks <- function(tracks) {
  ntracks <- NULL
  for(t in tracks)
    ntracks <- c(ntracks, simplifyTrack(t))
  ntracks
}
